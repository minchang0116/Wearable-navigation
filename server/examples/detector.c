...
#define INOTIFY_PATH "/home/jihoon/server"
#define BUF_LEN 128
...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        if(i%10000==0|| i == 8000 || i == 4000 || i == 6000 || (i < 1000 && i%100 == 0)){
#ifdef GPU
            if(ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            char buff[256];
...
//coding
static int __handle_inotify_event(const struct inotify_event *event)
{
