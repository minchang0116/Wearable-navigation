...
### 개요
시각장애인을 위한 웨어러블 내비게이션으로 라즈베리파이의 GPS값으로 목적지까지의 도보 정보를 음성으로 알려주는 서비스
...
| **사진** | **팀원**      | **역할**                                                     | **기여도** |
| -------- | ------------- | ------------------------------------------------------------ | ---------- |
|          | 팀장 고지훈 | 라즈베리파이 모듈(STT, TTS, HotWord 감지, GPS 등) 설치 및 클라이언트 코드 작성, AWS 설계 | 25       |
|          | 강민창 | 영상 처리 및 객체 감지, 전체적인 디버깅 담당, 내비게이션 로직 처리           | 25       |
|          | 김동욱 | 영상 처리 및 이미지 학습 개선 지원.디바이스 구성 지원.       | 25       |
|          | 박준수 | 내비게이션 구현하기 위한 AWS 설계 및 구현.                  | 25       |
